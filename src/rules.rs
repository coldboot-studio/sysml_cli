...
pub const CATALOG: &[Rule] = &[
    ...
    Rule {
        id: "SYSML212",
        name: "SuspectSelfSpecialization",
        short_description: "Feature appears to specialize itself.",
        full_description: "A feature uses `:>` or `specializes` to refer to an unqualified name that matches its own declared name. This MAY be a typo or it MAY be a legitimate redefinition of an inherited member with the same local name — token-level analysis cannot distinguish the cases. Surfaced as a warning so authors can review; AST-aware checking (US-201) will tighten this to an error for the unambiguous cases.",
        default_level: Severity::Warning,
    },
    Rule {
        id: "SYSML213",
        name: "SuspectSelfRedefinition",
        short_description: "Feature appears to redefine itself.",
        full_description: "A feature uses `:>>` or `redefines` with an unqualified target matching its own declared name. The common legitimate case is redefinition of an inherited member of the same name (e.g., `port po1 :>> po1` redefining the inherited `po1`). The bug case is a true self-redefinition. Token analysis cannot distinguish; warning level is conservative.",
        default_level: Severity::Warning,
    },
    Rule {
        id: "SYSML220",
